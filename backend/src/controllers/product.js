...
module.exports.createProduct = async (req, res, next) => {
  try {
    const { name, price, quantity, category, description, reviews } = req.body;
    let productPictures = [];
    if (req.files) {
      ...
      return res.status(200).json({
        ...
        productsByPrice: {
          under5k: products.filter(
            (singleProduct) => singleProduct.price < 5000
          ),
          under10k: products.filter(
            (singleProduct) => 5000 <= singleProduct < 10000
          ),
          under15k: products.filter(
            (singleProduct) => 10000 <= singleProduct < 15000
          ),
          under20k: products.filter(
            (singleProduct) => 15000 <= singleProduct < 20000
          ),
          under25k: products.filter(
            (singleProduct) => 20000 <= singleProduct < 25000
          ),
          under30k: products.filter(
            (singleProduct) => 25000 <= singleProduct < 30000
          ),
        },
      });
    } else {
