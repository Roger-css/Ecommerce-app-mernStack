...
const verifyJWT = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_ACCESS_TOKEN_KEY, (err, decode) => {
    if (err) {
      if (err.name === "TokenExpiredError") {
        const cookies = req.cookies;
        if (!cookies.jwt)
          return res.status(401).json({ message: `Unauthorized line19` });
        ...
        jwt.verify(
          ...
          async (err, decode) => {
            ...
            if (!foundUser)
              return res.status(401).json({ message: `Unauthorized line 28 ` });
